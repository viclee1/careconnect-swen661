...
- Maestro E2E flows (`mobile/maestro/*.yaml`) have now been run against a debug APK on a
  connected Android emulator: **5 of 6 flows pass**; the sixth
  (`03_send_message.yaml`) fails intermittently on a cold app launch. This was
  root-caused to an OS-level slow-cold-start condition (Android's own
  `ActivityTaskManager` logs "Launch timeout has expired, giving up wake lock!" during
  the failing runs), reproduced only when the test host was running two emulators and two
  Metro/Expo processes at once — an environmental resource-contention issue, not a defect
  in the flow, the screen, or its accessibility labeling. See
  [`docs/testing/maestro-results.xml`](docs/testing/maestro-results.xml) for the JUnit
  report from the last run and Known Limitations below. The original build blocker that
  had prevented any run (`npx expo run:android` failing on JDK 25 during the native
  `react-native-screens` CMake step) is unrelated and was separately root-caused as a
  JDK-25/26-vs-Gradle-9 native-toolchain incompatibility, fixed by pointing the build at
  JDK 17 — see "Building for Android" in `mobile/README.md`.
...
1. **No manual screen-reader pass performed for this client.** Unlike the Flutter
   ...
   outstanding.
2. **One Maestro flow (`03_send_message.yaml`) is intermittently flaky on cold app
   launch**, passing on most runs but occasionally failing an early `assertVisible` when
   the emulator's cold start is unusually slow. Confirmed via device evidence (a blank
   white screen at the failure moment, plus Android's own `ActivityTaskManager` logging a
   launch-timeout wake-lock warning) to be an OS/host-resource-contention issue — not a
   missing wait, not a code defect, and not an accessibility-labeling problem — reproduced
   only when the host was running two emulators and two Metro/Expo processes
   simultaneously. Adding an explicit 30s `extendedWaitUntil` did not fully eliminate it
   (2/5 runs still failed at the 30–32s boundary), which is further evidence the ceiling
   is host resource contention rather than a fixed insufficient timeout. The other 5 of 6
   flows, including the accessibility-focused one
   (`06_accessibility_screen_reader_navigation.yaml`), pass reliably. See
