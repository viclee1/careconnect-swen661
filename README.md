...
If your installed Expo SDK differs from the one this was built against (SDK 57 / React Native 0.86), run `npx expo install --fix` once after `npm install` to align the native module versions. `ios/` and `android/` are generated by `npx expo prebuild` and stay untracked while the project remains in the managed workflow. See [React Native mobile client](#react-native-mobile-client) for screens, architecture, and tests.
